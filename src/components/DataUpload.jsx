...
import WorkingCapitalOverview from './WorkingCapitalOverview';
...
export default function DataUpload({ cycleDate, setCycleDate }) {
  ...
  return (
    <div className="glass-panel p-6 md:p-8 space-y-8">
      ...
      {!cycleDate && (
        <div className="bg-amber-950/40 text-amber-200 p-4 rounded-lg text-sm flex items-center gap-3 border border-amber-800/60">
      ...
      {cycleDate && (
        <div className="pt-2">
          <WorkingCapitalOverview cycleDate={cycleDate} key={`${cycleDate}-${uploadCount}`} />
        </div>
      )}
    </div>
  );
